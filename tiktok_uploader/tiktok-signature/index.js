...
class Signer {
  ...
  constructor(default_url, userAgent, browser) {
    ...
    if (process.env.TIKTOK_PROXY) {
      // Playwright understands socks5://, not the socks5h:// variant requests/PySocks expect.
      this.options.proxy = { server: process.env.TIKTOK_PROXY.replace("socks5h://", "socks5://") };
    }
  }
